        ...
        try:
            ...
            with col2:
                ...
                if currency_option == '원화':
                    ...
                    # 외화는 USD로 가정
                    plot_df = df[df['환종'].isin(['USD', 'EUR', 'JPY', 'CNY'])].groupby('연령구분')['채권금액(외화)'].sum().reset_index()
                    amount_col = '채권금액(외화)'
                    title = "외화 채권 연령 현황"
                else: # 모든 통화
                ...
                if selected_point:
                    ...
                    summary_df = detail_df.groupby('거래처명')['채권금액(원화)'].sum().reset_index()
                    summary_df = summary_df.sort_values(by='채권금액(원화)', ascending=False)

                    # Top 5와 나머지 '기타'로 분리
                    top5_customers = summary_df.head(5)
                    if len(summary_df) > 5:
                        other_amount = summary_df.iloc[5:]['채권금액(원화)'].sum()
                        other_row = pd.DataFrame([{'거래처명': '기타', '채권금액(원화)': other_amount}])
                        final_df = pd.concat([top5_customers, other_row], ignore_index=True)
                    else:
                        final_df = top5_customers

                    # 최종 데이터프레임의 금액 포맷팅
                    final_df['채권금액(원화)'] = final_df['채권금액(원화)'].apply(lambda x: f'{x:,.0f}')
